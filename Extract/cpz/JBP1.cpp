	...
	)
{
	...
	if (*(LPDWORD)&pbtSrc[8] & 0x08000000) {
		for (int i = 0; i < 0x40; i++) quant_y[i] = *lpin++;
		for (int i = 0; i < 0x40; i++) quant_c[i] = *lpin++;
	}
	...
	)
{
	...
	do
	{
		...
		else
		{
			...
			x = c + d;
			x = ( x * 0x00008A8B ) >> 16;
			c = (( c * 0x0000C3EF ) >> 16) + x; //sar==unsignedだけど
	...
	)
{
	...
	// 高速化テーブル 切り捨て、飽和
	// -0x80 以下は 0, 0x80以上は0xFF 

	if( !is_tbl )
	{
		for(n=0; n<0x100; n++) fixed_byte_tbl[n]=0;

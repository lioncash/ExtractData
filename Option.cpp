...
LRESULT COption::StdProc(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp)
{
	...
	switch (msg) {
		case WM_INITDIALOG:
		{
			...
			// Listview settings
			ListGroup.Create(hWnd, _T("List Settings"), ID++, x, y += 20, 470, 75);
			ListLabelBk.Create(hWnd, _T("Background Color"), ID++, x + xx, y += 24, 50, 20);
			ListEditBk.Create(hWnd, pOption->szListBkColor, ID++, x + xx + 50, y - 4, 70, 22);
			ListEditBk.SetLimit(6);
			ListLabelText.Create(hWnd, _T("Text Color"), ID++, x + xx, y += 24, 50, 20);
			ListEditText.Create(hWnd, pOption->szListTextColor, ID++, x + xx + 50, y - 4, 70, 22);
			ListEditText.SetLimit(6);

			// Search Settings
			SearchGroup.Create(hWnd, _T("Files to be searched"), ID++, x, y += 40, 470, 100);
			//y += 20;
			...
			SearchBtn[0].Create(hWnd, _T("Select all"), ID++, 305, y += 30, 80, 20);
			SearchBtn[1].Create(hWnd, _T("Deselct"),   ID++, 385, y, 80, 20);

			// Search Accuracy Settings
			HighSearchGroup.Create(hWnd, _T("Search Accuracy"), ID++, x, y += 40, 470, 50);
			HighSearchCheckOgg.Create(hWnd, _T("Increase the accuracy of OGG searches"), ID++, x + xx, y += 20, 160, 20);
...
LRESULT COption::ExtractProc(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp)
{
	...
	{
		_T("Extract each folder"), _T("Fix the CRC of OGG files upon extraction"), _T("Enable simple decoding")
	};
	...
	switch (msg) {
		case WM_INITDIALOG:
		{
			...
			int y_Image = y;
			ExtractGroupImage.Create(hWnd, _T("Output image format"), ID++, x, y_Image += 34, 190, 130);
			...
			ExtractLabelPng.Create(hWnd, _T("Compression Level"), ID++, x + xx + 50, y_Image + 3, 60, 20);
			...
			ExtractCheckAlpha.Create(hWnd, _T("Enable alpha blending"), ID++, x + xx, y_Image += 22, 100, 20);
			ExtractCheckAlpha.SetCheck(pOption->bAlphaBlend);
			ExtractLabelAlpha.Create(hWnd, _T("Background color"), ID++, x + xx * 2, y_Image += 24, 40, 20);
			...
			ExtractCheckFastAlpha.Create( hWnd, _T("Faster processing"), ID++, x + xx * 2, y_Image += 20, 120, 20);
			...
			ExtractGroupSave.Create(hWnd, _T("Destination"), ID++, x_Save, y_Save += 34, 280, 110);
			ExtractRadioSave.Close();
			ExtractRadioSave.Create(hWnd, _T("Specify each time"), ID++, x_Save + xx, y_Save += 18, 200, 20);
			ExtractRadioSave.Create(hWnd, _T("Same folder as input source"), ID++, x_Save + xx, y_Save += 20, 200, 20);
			ExtractRadioSave.Create(hWnd, _T("The following folder"), ID++, x_Save + xx, y_Save += 20, 200, 20);
			...
			ExtractBtnSave.Create(hWnd, _T("Browse"), ID++, x_Save + xx * 2 + 200, y_Save + 1, 40, 20);
			...
			ExtractBtnTmp.Create(hWnd, _T("Browse"), ID++, x + 300, y - 3, 40, 20);
...
LRESULT COption::SusieProc(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp)
{
	...
	switch (msg) {
		case WM_INITDIALOG:
		{
			...
			SusieCheckUse.Create(hWnd, _T("Use Susie Plugins"), ID++, x, y += 20, 200, 20);
			...
			SusieBtnDir.Create(hWnd, _T("Browse"), ID++, x + xx + 275, y - 3, 40, 20);
			SusieBtnDir.Enable(pOption->bSusieUse);

			SusieCheckFirst.Create(hWnd, _T("Give Susie plugins priority when decoding"), ID++, x + xx, y += 20, 200, 20);
			...
			SusieBtnUpdate.Create(hWnd, _T("Update"), ID++, x + 240, y += 200, 50, 20);
			SusieBtnUpdate.Enable(pOption->bSusieUse);
			SusieBtn[0].Create(hWnd, _T("All ON"), ID++, x + 300, y, 80, 20);
			SusieBtn[0].Enable(pOption->bSusieUse);
			SusieBtn[1].Create(hWnd, _T("All OFF"), ID++, x + 380, y, 80, 20);
